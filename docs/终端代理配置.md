...
```
> [!NOTE]
>  在设置代理后，使用`sudo`执行其他命令时，需要给`sudo`加上`-E`参数才能使用刚刚配置的代理环境变量，
>
...
## 为git命令设置ssh代理
克隆ssh仓库（以`git@github.com:`开头）或对ssh仓库执行pull/push时，使用export设置http/https是无效的，正确的操作是设置ssh的代理。
...
Host github.com
  # ...（其他的配置）
  # 注意将下方 192.168.120.1:10808 更改为你的代理服务器地址
  ProxyCommand connect -S 192.168.120.1:10808 %h %p
  # ...（其他的配置）
```

#### **Linux**
安装`netcat`/`netcat-openbsd`/`netcat-traditional`三者之一
...
仅说明在linux上使用windows的代理的场景。

需要在Windows上的代理软件中开启**允许来自局域网的连接**
...
1. 在 `控制面板\系统和安全\Windows Defender 防火墙`中，点击左侧的高级设置打开高级安全防火墙，或者也可以按
<key>Win</key>+<key>R</key>`后输入`wf.msc`打开高级安全防火墙设置
2. 左侧点击入站规则，然后右侧点击新建规则，
    a. 规则类型选择`端口`=》
    b. 下一页，协议选择`TCP`，端口选择`特定本地端口`并输入`10808, 10809`（Clash是`7890`，具体请查看软件中的端口设置）=》
    c. 下一页，操作选择`允许连接`=》
    d. 下一页，配置文件三个多选框`域、专用、公用`都勾选上=》
    e. 最后一页，名称描述任意输入，点击完成即可

### 3. linux终端设置代理

假设windows代理主机A的地址为`192.168.1.12`，设备B的地址为`192.168.1.55`，
